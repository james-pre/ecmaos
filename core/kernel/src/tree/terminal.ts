          ...
          case CommandPath.KERNEL: // TODO: Limit the power of links and/or require user confirmation
            switch (commandPath[1]) {
              case "execute":
                ...
                globalThis?.kernel?.execute({
                  command: args["command"],
                  args: args["args"]?.split(" ") || [],
                  shell: globalThis?.kernel?.shell,
                });
                break;
            }
            break;
...
export class Terminal extends XTerm implements ITerminal {
  ...
  get addons() {
    return this._addons as Map<string, ITerminalAddon>;
  }
  get ansi() {
    return this._ansi;
  }
  ...
  set promptTemplate(value: string) {
  ...
  constructor(options: TerminalOptions = DefaultTerminalOptions) {
    ...
    this.onKey(({ key }) => {
      if (key && key.length > 0) {
        if (this._stdinController)
          this._stdinController.enqueue(new TextEncoder().encode(key));
      }
    });
    ...
    globalThis.addEventListener("resize", () => {
      if (this.addons?.get("fit")) (this.addons.get("fit") as FitAddon).fit();
      this.events.dispatch<TerminalResizeEvent>(TerminalEvents.RESIZE, {
        cols: this.cols,
        rows: this.rows,
      });
    });
    ...
    if (options.socket) {
      ...
      this._socket.addEventListener("message", (event) => {
        ...
        this._kernel.events.dispatch<TerminalMessageEvent>(
          TerminalEvents.MESSAGE,
          { terminal: this, message: event },
        );
      ...
      const attachSocket = async () => {
        ...
        if (this._socket) {
          try {
            ...
            this._kernel.events.dispatch<TerminalAttachEvent>(
              TerminalEvents.ATTACH,
              { terminal: this, socket: this._socket },
            );
          } catch (err) {
            console.error(err);
    ...
    this._history = this._kernel.storage.local.getItem("history")
      ? JSON.parse(this._kernel.storage.local.getItem("history") || "[]")
      : [];
    this._historyPosition = this._history.length;

    this.events.dispatch<TerminalCreatedEvent>(TerminalEvents.CREATED, {
      terminal: this,
    });
  }
  ...
  mount(element: HTMLElement) {
    ...
    this.events.dispatch<TerminalMountEvent>(TerminalEvents.MOUNT, {
      terminal: this,
      element,
    });
  }
  ...
  listen() {
    this.unlisten();
    this._keyListener = this.onKey(this.keyHandler.bind(this));
    this.events.dispatch<TerminalListenEvent>(TerminalEvents.LISTEN, {
      terminal: this,
    });
  }
  ...
  unlisten() {
    ...
    this.events.dispatch<TerminalUnlistenEvent>(TerminalEvents.UNLISTEN, {
      terminal: this,
    });
  }
  ...
  async shortcutKeyHandler({ domEvent }: { domEvent: KeyboardEvent }) {
    this.events.dispatch<TerminalKeyEvent>(TerminalEvents.KEY, {
      key: domEvent.key,
      domEvent,
    });
        ...
        switch (domEvent.key) {
          ...
          case "Backspace":
            if (cursor > 0) {
              ...
              this.write(
                ansi.cursor.horizontalAbsolute(0) +
                  ansi.erase.inLine(2) +
                  ":" +
                  input,
              );
          ...
          default:
            if (
              ...
              domEvent.key.match(/^[a-zA-Z0-9]$/)
            ) {
              input =
                input.slice(0, cursor) + domEvent.key + input.slice(cursor);
              if (!hide)
                this.write(
                  ansi.cursor.horizontalAbsolute(0) +
                    ansi.erase.inLine(2) +
  ...
  prompt(text: string = this._promptTemplate) {
    ...
    // @ts-expect-error
    return (
      this.ansi.style[this.options.theme?.promptColor || "green"] +
      text
  ...
  override async paste(data?: string) {
    ...
    this.events.dispatch<TerminalPasteEvent>(TerminalEvents.PASTE, {
      text: clip,
    });
    this.events.dispatch<TerminalInputEvent>(TerminalEvents.INPUT, {
      terminal: this,
      data: clip,
    });
  }
  ...
  async keyHandler({
    ...
    if (domEvent.ctrlKey) {
      switch (keyName) {
        case "c":
          this.events.dispatch<TerminalInterruptEvent>(
            TerminalEvents.INTERRUPT,
            { terminal: this },
          );
          this._cmd = "";
        ...
        case "Escape":
          return this.write(
            ansi.erase.display(2) + ansi.cursor.position() + this.prompt(),
          );
      }
    }
    ...
    switch (keyName) {
      case "Enter":
        ...
        if (this._cmd.trim().length > 0) {
          ...
          if (
            this._cmd[0] !== " " &&
            this._cmd !== this._history[this._history.length - 1]
          ) {
            this._history.push(this._cmd);
            try {
              this._kernel.storage.local.setItem(
                "history",
          ...
          try {
            this.events.dispatch<TerminalExecuteEvent>(TerminalEvents.EXECUTE, {
              terminal: this,
              command: this._cmd,
            });
            const result = await this._shell.execute(this._cmd);
            if (result === -1)
              throw new Error(`Command not found: ${this._cmd}`);
          } catch (error) {
      ...
      case "Backspace":
        if (this._cursorPosition > 0) {
          this._cmd =
            this._cmd.slice(0, this._cursorPosition - 1) +
            this._cmd.slice(this._cursorPosition);
      ...
      case "Delete":
        if (this._cursorPosition < this._cmd.length) {
          this._cmd =
            this._cmd.slice(0, this._cursorPosition) +
            this._cmd.slice(this._cursorPosition + 1);
          ...
          if (this._multiLineMode) {
            ...
            if (parts.length > 1) {
              this.write(
                "> " + parts[0] + chalk.gray("#" + parts.slice(1).join("#")),
              );
            } else {
          ...
          if (this._cursorPosition < this._cmd.length)
            this.write(`\x1b[${this._cmd.length - this._cursorPosition}D`);
        }
        break;
      case "ArrowUp":
      ...
      case "End":
        ...
        this.write(
          ansi.cursor.horizontalAbsolute(
            this.prompt().length + this._cmd.length + 1,
          ),
        );
        break;
      case "Tab": {
        ...
        if (matches.length > 0) {
          ...
          this._tabCompletionIndex =
            (this._tabCompletionIndex + 1) % matches.length;
  ...
  serialize() {
    if (this.addons?.get("serialize"))
      return (this.addons.get("serialize") as SerializeAddon).serialize();
    else return null;
  }
  ...
  override write(data: string | Uint8Array) {
    super.write(data);
    this.events.dispatch<TerminalWriteEvent>(TerminalEvents.WRITE, {
      text: data instanceof Uint8Array ? new TextDecoder().decode(data) : data,
    });
  }

  override writeln(data: string | Uint8Array) {
    super.writeln(data);
    this.events.dispatch<TerminalWritelnEvent>(TerminalEvents.WRITELN, {
      text: data instanceof Uint8Array ? new TextDecoder().decode(data) : data,
    });
  }
  ...
  restoreCommand(cmd: string) {
    ...
    if (parts.length > 1) {
      this.write(
        this.prompt() + parts[0] + chalk.gray("#" + parts.slice(1).join("#")),
      );
    } else {
  ...
  private getCompletionMatches(partial: string): string[] {
    ...
    const searchDir =
      lastSlashIndex !== -1
        ? path.resolve(
            this._shell.cwd,
            lastWord.substring(0, lastSlashIndex + 1),
          )
        : this._shell.cwd;
    const searchTerm =
      lastSlashIndex !== -1 ? lastWord.substring(lastSlashIndex + 1) : lastWord;
    ...
    try {
      ...
      return matches.map((match: string) => {
        ...
        const isDirectory = this._kernel.filesystem.fsSync
          .statSync(fullPath)
          .isDirectory();
        const escapedMatch = match.includes(" ")
          ? match.replace(/ /g, "\\ ")
          : match;
  ...
  start() {
    ...
    const interval = setInterval(() => {
      ...
      const fullWidth = [...fullText].reduce((width, char) => {
        if (/\p{Emoji}/gu.test(char)) return width + 1; // Emoji generally takes 2 spaces
        if (
          /[\u3000\u3001-\u303F]|[\u3040-\u309F]|[\u30A0-\u30FF]|[\uFF00-\uFFEF]|[\u4E00-\u9FAF]|[\u2605-\u2606]|[\u2190-\u2195]|\u203B/gu.test(
            char,
          )
        )
          return width + 2; // Wide characters
